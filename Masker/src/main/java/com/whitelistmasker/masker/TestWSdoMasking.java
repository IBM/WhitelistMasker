...
public class TestWSdoMasking implements Serializable {
	...
	public JSONObject getMaskedContent() {
		...
		String unmaskedFile = MaskerConstants.Masker_DIR_PROPERTIES + "Unmasked.txt";
		...
		while (true) {
			...
			String filename = MaskerUtils.prompt(
					"Enter the fully qualified filename of the text to be masked (" + unmaskedFile + ") or q to quit:");
			...
			String test = MaskerUtils.prompt("Should numbers be masked, or q to quit(Y)");
			if ("q".equals(test)) {
				break;
			}
			if (test.length() == 0) {
			...
			String hostname = MaskerUtils
					.prompt("Enter the hostname where the service is running or q to quit (" + servicename + ")");
			...
			try {
				List<String> unmaskedLines = MaskerUtils.loadTextFile(filename);
				...
				service.put("endpoint", "/" + propFile.getProperty("servletname", "MaskWebServices") + "/"
						+ propFile.getProperty("version", "v1") + "/masker/doMasking");
				...
				System.err.println("Note: you can check the server is running by pasting this in a browser:\n" + "http://"
						+ hostname + ":9080/MaskWebServices/v1/HelloMasker");
			}
		}
		return response;
	}
