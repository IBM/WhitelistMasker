...
COPY --chown=1002:0 MaskWebServices/server.xml /config/

RUN features.sh

COPY --chown=1002:0 MaskWebServices/target/*.war /config/dropins/
COPY --chown=1002:0 MaskWebServices/properties/  /opt/ol/wlp/output/defaultServer/properties/
# COPY --chown=1002:0 MaskWebServices/server.env /config/
COPY --chown=1002:0 Masker/properties/  /opt/ol/wlp/output/defaultServer/properties/

# copy the .jar containing the utility into the appropriate place (relative to properties directory)
COPY --chown=1002:0 Masker/target/Masker-1.2.4-jar-with-dependencies.jar /opt/ol/wlp/output/defaultServer/Masker-1.2.4-jar-with-dependencies.jar
